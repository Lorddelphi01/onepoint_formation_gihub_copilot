...
Le bloc de base d'un workflow n8n : une étape unique (déclencheur, appel d'API, filtre, mise en forme de données...) que l'on relie à d'autres nœuds pour former un workflow complet. Présenté au Chapitre 18.
...
Une URL générée par un service (ici, un nœud Déclencheur n8n) qui écoute les requêtes HTTP entrantes et déclenche une action dès qu'elle en reçoit une, plutôt que d'être interrogée activement. Présenté au Chapitre 18.
